...
var DatatablesButtonsCashReport1 = function () {
	...
	var initTableCashReport1 = function () {
		...
		var table = $('#datatable_CashReport1').DataTable({
			...
			stateSave: false,
			ajax: {
				url: window.location.protocol + '//' + window.location.hostname + ':' + window.location.port + '/Report/GetCashReport1?collectionDefinitionTypeId=-99',
				type: 'POST',
				dataSrc: '',
			},
...
var DatatablesButtonsCashReport1DetailCollection = function () {
	...
	var initTableCashReport1DetailCollection = function () {
		...
		$('#export_print_detail').on('click', function (e) {
			e.preventDefault();
			table.button(0).trigger();
		});

		$('#export_copy_detail').on('click', function (e) {
			e.preventDefault();
			table.button(1).trigger();
		});

		$('#export_excel_detail').on('click', function (e) {
			e.preventDefault();
			table.button(2).trigger();
		});

		$('#export_csv_detail').on('click', function (e) {
			e.preventDefault();
			table.button(3).trigger();
		});

		$('#export_pdf_detail').on('click', function (e) {
			e.preventDefault();
			table.button(4).trigger();
		});
